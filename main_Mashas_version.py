...
def get_performance(data, user_ratings, initial_cluster, est_cluster):  # Assumes user_ratings is a dataframe with entity_id and rating as columns
                                                 # takes the initial cluster of the user and estimated cluster g_hat, 
                                                 # that should be also spit out by the algorithm
    # Accuracy
    ...
    regret = 0
    for i in range(len(user_ratings_sorted)): # TO DO:
                                       # How many items are rated? Do we calculate regret for all of them?
                                       # Didn't fully understand the algorithm, we need the numb of rated items
...
def clusterBasedBanditAlgorithm(B, C, D, G, mean, var): # Algorithm 1
    ...
    n = len(list(mean.columns[1:]))  # number of columns (books) (without user_id)

    for i in range(n):
        M_n = define_candidate_set(V_n, g_hat, G, mean, var) # defines the books that are given to the user for rating
        if M_n is not None:
            g_hat = g_hat_select()
            V_n = g_exploration(V_n, g_hat, G, mean, var)
...
def evaluate(dataset_n, ngroups = 4):        # MASHA: added ngroups, for the number of clusters we want to create
                                             # we have to then test performance on 4, 8, 16 and 32 groups
    print("Evaluating dataset " + str(dataset_n))
    data = import_dataset(dataset_n)
    overall_accuracy = []
    overall_convergence_time = []

    for i in range(1, ngroups+1):            # MASHA: changed the loop so it takes the groups and then generates 1000 users per group
        for u in tqdm(range(1000), desc = "Running the algorithm for different users", total=1000):
        # generating 1000 new users per group
            clusters = clusterUsers(data, ngroups)
            ...
            # MASHA: in the algorithm there should be an option to predefine an initial cluster and generate
            # new ratings based on it, then based on the ratings a cluster is estimated and spit out in g_hat
            accuracy, convergence_time = get_performance(data, user_ratings, initial_cluster=i, est_cluster=g_hat)
